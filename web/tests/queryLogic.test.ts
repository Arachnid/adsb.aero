...
const POLYGON: [number, number][] = [[-2, 50], [2, 50], [2, 52], [-2, 52], [-2, 50]];
...
describe("isPredValid", () => {
  describe("aircraft", () => {
    ...
    it("valid with icao types", () => {
      expect(isPredValid({ id: "1", kind: "aircraft", icaoTypes: ["B738"], emitters: [] })).toBe(true);
    });
    it("valid with emitters", () => {
      expect(isPredValid({ id: "1", kind: "aircraft", icaoTypes: [], emitters: ["A3"] })).toBe(true);
    });
  });
  ...
  describe("starts_within / ends_within", () => {
    it("none shape invalid without time", () => {
      expect(isPredValid({ id: "1", kind: "starts_within", shape: "none", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "" })).toBe(false);
    });
    it("none shape valid with timeFrom", () => {
      expect(isPredValid({ id: "1", kind: "starts_within", shape: "none", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "2025-01-01T00:00", timeTo: "" })).toBe(true);
    });
    it("none shape valid with timeTo", () => {
      expect(isPredValid({ id: "1", kind: "ends_within", shape: "none", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "2025-01-02T00:00" })).toBe(true);
    });
    it("viewport always valid", () => {
      expect(isPredValid({ id: "1", kind: "starts_within", shape: "viewport", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "" })).toBe(true);
    });
    it("circle invalid without coordinates", () => {
      expect(isPredValid({ id: "1", kind: "starts_within", shape: "circle", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "" })).toBe(false);
    });
    it("circle valid with coordinates", () => {
      expect(isPredValid({ id: "1", kind: "starts_within", shape: "circle", lat: 51.5, lng: -0.1, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "" })).toBe(true);
    });
    it("polygon invalid without polygon", () => {
      expect(isPredValid({ id: "1", kind: "ends_within", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "" })).toBe(false);
    });
    it("polygon invalid with fewer than 3 points", () => {
      expect(isPredValid({ id: "1", kind: "ends_within", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: [[-2, 50], [2, 50]], timeFrom: "", timeTo: "" })).toBe(false);
    });
    it("polygon valid with 3+ points", () => {
      expect(isPredValid({ id: "1", kind: "ends_within", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: POLYGON, timeFrom: "", timeTo: "" })).toBe(true);
    });
  });

  describe("region", () => {
    it("none shape invalid without time", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "none", lat: null, lng: null, radiusNm: 25, polygon: null, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(false);
    });
    it("none shape valid with time", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "none", lat: null, lng: null, radiusNm: 25, polygon: null, altMin: null, altMax: null, timeFrom: "2025-01-01T00:00", timeTo: "", squawkCodes: [] })).toBe(true);
    });
    it("viewport always valid", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "viewport", lat: null, lng: null, radiusNm: 25, polygon: null, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(true);
    });
    it("circle invalid without coordinates", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "circle", lat: null, lng: null, radiusNm: 25, polygon: null, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(false);
    });
    it("circle valid with coordinates", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "circle", lat: 51.5, lng: -0.1, radiusNm: 25, polygon: null, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(true);
    });
    it("polygon invalid without polygon", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: null, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(false);
    });
    it("polygon valid with 3+ points", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: POLYGON, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(true);
    });
    it("invalid when altMin > altMax", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "circle", lat: 51.5, lng: -0.1, radiusNm: 25, polygon: null, altMin: 40000, altMax: 10000, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(false);
    });
    it("valid when altMin === altMax", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "circle", lat: 51.5, lng: -0.1, radiusNm: 25, polygon: null, altMin: 35000, altMax: 35000, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(true);
    });
    it("valid when only altMin set", () => {
      expect(isPredValid({ id: "1", kind: "region", regionName: "R", shape: "circle", lat: 51.5, lng: -0.1, radiusNm: 25, polygon: null, altMin: 35000, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] })).toBe(true);
    });
  });
});
...
describe("isGroupValid", () => {
  ...
  it("nested group: all valid", () => {
    ...
    expect(isGroupValid(group(
      inner,
      { id: "2", kind: "aircraft", icaoTypes: ["A320"], emitters: [] },
    ))).toBe(true);
  });
});
...
describe("computeLabels", () => {
  ...
  it("circle preds are numbered per type", () => {
    const g = group(
      ...
      { id: "s2", kind: "starts_within", shape: "circle", lat: 52, lng: 1, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" },
      { id: "e1", kind: "ends_within", shape: "circle", lat: 53, lng: 2, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" },
    );
    const labels = computeLabels(g);
  ...
  it("viewport preds get base name only, not counted", () => {
    const g = group(
      { id: "v1", kind: "starts_within", shape: "viewport", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "" },
      { id: "c1", kind: "starts_within", shape: "circle", lat: 51, lng: 0, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" },
    );
    const labels = computeLabels(g);
  ...
  it("none preds get base name only, not counted", () => {
    const g = group(
      { id: "n1", kind: "starts_within", shape: "none", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "2025-01-01T00:00", timeTo: "" },
      { id: "c1", kind: "starts_within", shape: "circle", lat: 51, lng: 0, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" },
    );
    const labels = computeLabels(g);
  ...
  it("region uses 'Ever' base name", () => {
    const g = group(
      { id: "r1", kind: "region", regionName: "R", shape: "circle", lat: 51, lng: 0, radiusNm: 10, polygon: null, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] },
      { id: "r2", kind: "region", regionName: "R", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: POLYGON, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] },
    );
    const labels = computeLabels(g);
  ...
  it("always_within uses 'Always' base name", () => {
    const g = group(
      { id: "a1", kind: "always_within", regionName: "R", shape: "circle", lat: 51, lng: 0, radiusNm: 10, polygon: null, altMin: null, altMax: null, timeFrom: "", timeTo: "", squawkCodes: [] },
    );
    const labels = computeLabels(g);
...
describe("collectGeometries", () => {
  ...
  it("skips viewport and none shapes", () => {
    const g = group(
      { id: "1", kind: "starts_within", shape: "viewport", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "" },
      { id: "2", kind: "starts_within", shape: "none", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "2025-01-01T00:00", timeTo: "" },
    );
    expect(collectGeometries(g)).toEqual([]);
  });
  ...
  it("includes circle geometry", () => {
    const g = group(
      { id: "c1", kind: "starts_within", shape: "circle", lat: 51.5, lng: -0.1, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" },
    );
  ...
  it("includes polygon geometry", () => {
    const g = group(
      { id: "p1", kind: "ends_within", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: POLYGON, timeFrom: "", timeTo: "" },
    );
    const [geom] = collectGeometries(g);
  ...
  it("labels are numbered per type, matching computeLabels", () => {
    const g = group(
      { id: "s1", kind: "starts_within", shape: "circle", lat: 51, lng: 0, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" },
      { id: "s2", kind: "starts_within", shape: "circle", lat: 52, lng: 1, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" },
      { id: "e1", kind: "ends_within", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: POLYGON, timeFrom: "", timeTo: "" },
    );
  ...
  it("assigns distinct colours cycling through palette", () => {
    const items = Array.from({ length: 7 }, (_, i) => ({
      ...
      lat: 51, lng: 0, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "",
    }));
  ...
  it("walks nested groups", () => {
    ...
    const g = group(inner, { id: "e1", kind: "ends_within", shape: "polygon", lat: null, lng: null, radiusNm: 25, polygon: POLYGON, timeFrom: "", timeTo: "" });
    const geoms = collectGeometries(g);
...
describe("anyViewportFilter", () => {
  ...
  it("false when no viewport shapes", () => {
    const g = group({ id: "1", kind: "starts_within", shape: "circle", lat: 51, lng: 0, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" });
    expect(anyViewportFilter(g)).toBe(false);
  });

  it("true when any pred has viewport shape", () => {
    const g = group(
      { id: "1", kind: "starts_within", shape: "circle", lat: 51, lng: 0, radiusNm: 10, polygon: null, timeFrom: "", timeTo: "" },
      { id: "2", kind: "ends_within", shape: "viewport", lat: null, lng: null, radiusNm: 25, polygon: null, timeFrom: "", timeTo: "" },
    );
